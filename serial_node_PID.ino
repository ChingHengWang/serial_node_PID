...
//double omega_left_target = 1.74;
//double omega_right_target = 1.74;
...
void setup() 
{
  ...
  pinMode(Encoder1A, INPUT);  pinMode(Encoder1B, INPUT);  pinMode(Encoder2A, INPUT);  pinMode(Encoder2B, INPUT);
  
  digitalWrite(Encoder1A, HIGH);  digitalWrite(Encoder1B, HIGH);  digitalWrite(Encoder2A, HIGH);  digitalWrite(Encoder2B, HIGH);       // turn on pullup resistor

  attachInterrupt(0, left_encoder, CHANGE); attachInterrupt(1, left_encoder, CHANGE); attachInterrupt(4, right_encoder, CHANGE); attachInterrupt(5, right_encoder, CHANGE);
...
void loop() 
{
     ...
     {                                    // enter tmed loop
        ...
        //PWM_val1= constrain(int(omega_left_target/0.00682352941), 0 , 255);
        //PWM_val2= constrain(int(omega_right_target/0.00682352941), 0 , 255); 
        if (omega_left_target<=0)   { analogWrite(motorIn1,PWM_val1);  analogWrite(motorIn2,0);}
        if (omega_left_target>0)    { analogWrite(motorIn1,0);  analogWrite(motorIn2,PWM_val1);}
        if (omega_right_target<=0)  { analogWrite(motorIn3,PWM_val2);  analogWrite(motorIn4,0);}
        if (omega_right_target>0)   { analogWrite(motorIn3,0);  analogWrite(motorIn4,PWM_val2);}
        //printMotorInfo();
     }
...
{                               
  ...
  omega_left_actual  = ((Encoder1pos - Encoder1posPre)*(1000/LOOPTIME))*2*PI/(CPR*gear_ratio);  //ticks/s to rad/s
  omega_right_actual = ((Encoder2pos - Encoder2posPre)*(1000/LOOPTIME))*2*PI/(CPR*gear_ratio);  //ticks/s to rad/s
...
{            
  ...
  sum_error = sum_error + error * LOOPTIME;
  d_error = (error - last_error) / LOOPTIME;
...
void printMotorInfo()  
{                                                                         
   Serial.print("  LEFT rad/s:");                  Serial.print(omega_left_actual);
   Serial.println();
   Serial.print("  RIGHT rad/s:");                  Serial.print(omega_right_actual);
   Serial.println();
